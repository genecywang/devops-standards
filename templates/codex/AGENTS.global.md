# AGENTS.md（Global DevOps / Platform Engineer Baseline）

本檔設計給個人快速同步使用，建議 symlink 到 `~/.codex/AGENTS.md`。它是全域 baseline，不取代各 repo 的 project-local `AGENTS.md`；若目前 repo 有自己的 `AGENTS.md`，以更接近工作目錄的 repo 指引補充或覆蓋本檔。

## 語言與排版

- 一律使用繁體中文回覆，技術術語保留英文原文。
- 中文與英文 / 數字間加半形空格。
- 回答以工程判斷為主，直接指出風險與替代方案。
...
Codex subagents 只在使用者明確要求，或任務確定彼此獨立且不碰 production / shared 狀態時使用。Subagent 不得自動擴權。
