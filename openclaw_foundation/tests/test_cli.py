...
def test_main_with_pod_cpu_usage_fixture_prints_success_response(capsys) -> None:
    project_root = Path(__file__).resolve().parents[1]
    exit_code = main(
        [
            "--fixture",
            str(project_root / "fixtures" / "pod_cpu_usage_request.json"),
            "--provider",
            "fake",
        ]
    )
    ...
    assert exit_code == 0
    assert '"result_state": "success"' in captured.out
    assert "get_pod_cpu_usage" in captured.out


def test_main_with_deployment_restart_rate_fixture_prints_success_response(capsys) -> None:
    project_root = Path(__file__).resolve().parents[1]
    exit_code = main(
        [
            "--fixture",
            str(project_root / "fixtures" / "deployment_restart_rate_request.json"),
            "--provider",
            "fake",
        ]
    )
    ...
    assert exit_code == 0
    assert '"result_state": "success"' in captured.out
    assert "get_deployment_restart_rate" in captured.out


def test_main_with_pod_logs_fixture_prints_success_response(capsys) -> None:
    project_root = Path(__file__).resolve().parents[1]
    exit_code = main(
        [
            "--fixture",
            str(project_root / "fixtures" / "pod_logs_request.json"),
            "--provider",
            "fake",
        ]
    )
    ...
    assert exit_code == 0
    assert '"result_state": "success"' in captured.out
    assert "get_pod_logs" in captured.out
